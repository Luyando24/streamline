          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-slate-700">Items that need your attention</p>
              </div>
          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm relative overflow-hidden">
            {!isAuthorized && (
              <div className="absolute inset-0 z-10 bg-white/60 backdrop-blur-[2px] flex items-center justify-center p-6 text-center">
                <div className="max-w-[220px] animate-in fade-in zoom-in duration-500">
                  ...
                  <p className="text-[10px] text-slate-700 leading-relaxed font-bold">Financial health analytics are reserved for Organization Administrators and Owners.</p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-slate-700">Income vs. Expenses — Last 6 months</p>
              </div>
              <select className="text-xs font-bold bg-slate-50/80 border rounded-lg px-3 py-1.5 focus:outline-none">
              ...
              {(isAuthorized ? data.cashFlowData : [1,2,3,4,5,6]).map((d: any, i: number) => {
                if (!isAuthorized) return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1 opacity-20">
                    <div className="w-full flex gap-1 items-end h-[180px]">
                      ...
                      <div className="flex-1 bg-slate-50/80 rounded-t-md" style={{ height: `${20 + Math.random() * 40}%` }} />
                    </div>
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[10px] font-medium text-slate-600">{d.month}</span>
                  </div>
                )
              })}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            <h3 className="text-sm font-bold text-brand-navy mb-4 flex items-center gap-2">
              <CalendarDays className="h-4 w-4 text-slate-600" /> Upcoming Deadlines
            </h3>
          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            <h3 className="text-sm font-bold text-brand-navy mb-4 flex items-center gap-2">
              <Clock className="h-4 w-4 text-slate-600" /> Recent System Logs
            </h3>
            <div className="space-y-4">
              {data.recentActivity.length > 0 ? data.recentActivity.map((log: any, i: number) => (
                ...
                <p className="text-[10px] text-slate-600 italic">No recent activity found</p>
              )}
            </div>
          </div>
  ...
  return (
    <div className="rounded-2xl border bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-4">
        <div className="h-10 w-10 rounded-xl bg-slate-50/80 flex items-center justify-center text-slate-600">
          {icon}
        </div>
      ...
      <h3 className="text-xs font-medium text-slate-700 mb-1">{title}</h3>
...
function TaskItem({ title, subtitle, type, urgent }: { title: string; subtitle: string; type: string; urgent: boolean }) {
  return (
    <div className={`flex items-start gap-3 p-3 rounded-xl border transition-all hover:shadow-sm ${urgent ? 'border-red-200 bg-red-50/50' : 'border-slate-200 bg-slate-50/80/50 hover:bg-white'}`}>
