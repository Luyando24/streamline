    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-700 font-medium">Configure compensation and statutory identifiers for your team.</p>
        </div>
      </div>
          ...
          <div key={e.profile_id} className="group p-6 bg-white border-2 border-slate-200 rounded-2xl hover:border-brand-blue-deep/20 hover:shadow-lg transition-all flex items-center justify-between gap-6">
            <div className="flex items-center gap-6">
               ...
               <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">{e.profiles.email}</p>
               </div>
            </div>
            ...
            <div className="flex items-center gap-12">
               ...
               <button 
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-slate-50/80 text-[10px] font-black uppercase tracking-widest text-slate-700 rounded-xl group-hover:bg-brand-navy group-hover:text-white transition-all shadow-sm active:scale-95"
               >
      ...
      {/* Centered Modal for Management */}
      <IndustrialModal
        isOpen={!!selectedProfile}
        onClose={() => setSelectedProfile(null)}
        title={selectedProfile?.profiles?.full_name || "Employee Config"}
        subtitle="Employee Configuration"
        ...
        {selectedProfile && (
          <form onSubmit={handleUpdate} className="space-y-8">
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-300 border-b border-slate-200 pb-2">Compensation</h3>
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-600 pl-1">Monthly Basic Salary (ZMW)</label>
                  <div className="relative">
                    <Banknote className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-600" />
                    <input 
                      name="basic_salary"
                      type="number" 
                      step="0.01"
            ...
            {/* Statutory Section */}
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-300 border-b border-slate-200 pb-2">Statutory Identifiers</h3>
            ...
            {/* Banking Section */}
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-300 border-b border-slate-200 pb-2">Banking Details</h3>
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-2">
